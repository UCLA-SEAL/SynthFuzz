...
from grammarinator.pkgdata import __version__
from grammarinator.tool.g4 import ANTLRv4Lexer, ANTLRv4Parser
from jinja2 import Environment
from mlirmut.synthfuzz.generator import InsertMatchPattern, QuantifierSpec
...
class Node(object):
    ...
    def print_tree(self):
        def _walk(node):
            nonlocal indent
            print(
                f'{"  " * indent}{str(node)}{"" if node not in visited else " (...recursion)"}'
            )
            if node in visited:
                return
...
class AlternationNode(Node):
    ...
    def simple_alternatives(self):
        ...
        if not self.out_neighbours or any(
            len(alt.out_neighbours) != 1
        ...
        simple_lits = [
            (
                alt.out_neighbours[0].src
                if isinstance(alt.out_neighbours[0], LiteralNode)
                else None
            )
            for alt in self.out_neighbours
        ]
        ...
        simple_rules = [
            (
                alt.out_neighbours[0].id
                if isinstance(alt.out_neighbours[0], RuleNode)
                else None
            )
            for alt in self.out_neighbours
        ]
    ...
    def __str__(self):
        return (
            f'{super().__str__()}; idx: {self.idx}; cond: {", ".join(self.conditions)}'
        )
...
class AlternativeNode(Node):
    ...
    def __init__(self, rule_id, alt_idx, idx):
        ...
        self.rule_id = rule_id  # Identifier of the container rule.
        self.alt_idx = (
            alt_idx  # Index of the container alternation inside the container rule.
        )
...
class GrammarGraph(object):
    ...
    def rules(self):
        return (
            vertex for vertex in self.vertices.values() if isinstance(vertex, RuleNode)
        )
    ...
    def imag_rules(self):
        return (
            ...
            if isinstance(vertex, ImagRuleNode)
        )

    def print_tree(self, root=None):
        if not root and not self.default_rule:
            raise ValueError(
                "Either `root` must be defined or `print` should be called after `default_rule` is set."
            )
    ...
    def calc_min_depths(self):
        ...
        while changed:
            ...
            for ident, node in self.vertices.items():
                ...
                min_depth = selector(
                    (
                        min_depths[out_node.id] + int(isinstance(out_node, RuleNode))
                        for out_node in node.out_neighbours
                        if not isinstance(out_node, QuantifierNode) or out_node.min > 0
                    ),
                    default=0,
                )
...
class ProcessorTool(object):
    ...
    def process(
        ...
        *,
        ...
        # Derive insert patterns for SynthFuzz
        with open(join(self._work_dir, "graph.pkl"), "wb") as f:
            pickle.dump(graph, f)

        insert_patterns = self.derive_insert_patterns(graph)
        with open(join(self._work_dir, "insert_patterns.pkl"), "wb") as f:
    ...
    def derive_insert_patterns(self, graph):
        parser_rules = [
            rule for rule in graph.rules if isinstance(rule, UnparserRuleNode)
        ]
        print(f"# parser rules: {len(parser_rules)}")

        def is_simple_quantifier(node: QuantifierNode):
            return len(node.out_neighbours) == 1 and isinstance(
                node.out_neighbours[0], UnparserRuleNode
            )

        def contains_quantifier(rule):
            quantifiers = [
                node for node in rule.out_neighbours if isinstance(node, QuantifierNode)
            ]
            if not quantifiers:
                return False
        ...
        parser_rules_with_quants = [
            rule for rule in parser_rules if contains_quantifier(rule)
        ]
        print(
            f"# parser rules with simple quantifiers: {len(parser_rules_with_quants)}"
        )
        ...
        for rule in parser_rules_with_quants:
            ...
            for child in rule.out_neighbours:
                if isinstance(child, QuantifierNode):
                    ...
                    child_rule = child.out_neighbours[0]
                    if not isinstance(child_rule, UnparserRuleNode):
                        print(rule.name)
                        raise ValueError(
                            f"Quantifier pattern expected to contain a rule, but found {type(child_rule)}"
                        )
                    match_pattern.append(
                        QuantifierSpec(
                            min=child.min,
                            max=child.max if child.max != "inf" else inf,
                            rule_name=child_rule.name,
                        )
                    )
            ...
            if valid_rule:
                insert_patterns[rule.name] = InsertMatchPattern(
                    match_pattern, child_rules
                )
    ...
    def _parse_grammar(self, grammar, encoding, errors, lib_dir):
        ...
        while work_list:
            ...
            antlr_parser = ANTLRv4Parser(
                CommonTokenStream(
                    ANTLRv4Lexer(FileStream(grammar, encoding=encoding, errors=errors))
                )
            )
    ...
    def _build_graph(actions, lexer_root, parser_root, options, default_rule):
        ...
        def chars_from_set(node):
            ...
            if node.TOKEN_REF():
                ...
                assert (
                    graph.vertices[src].start_ranges is not None
        ...
        def argActionBlock(node):
            ...
            if node and node.argActionBlock():
                for arg in "".join(
                    ...
                    args[arg_name.strip()] = (
                        arg_value.strip() if arg_value else arg_value
                    )
            return args
        ...
        def build_rule(rule, node):
            ...
            def build_expr(node, parent_id):
                if isinstance(node, ANTLRv4Parser.ParserRuleSpecContext):
                    ...
                    rule_node = UnparserRuleNode(
                        name=rule.name,
                        label=str(
                            node.identifier().TOKEN_REF()
                            or node.identifier().RULE_REF()
                        ),
                    )
                    ...
                    quant_id = graph.add_node(
                        QuantifierNode(
                            rule_id=rule.name, idx=quant_idx, **quant_ranges[suffix]
                        )
                    )
                    quant_idx += 1
                    ...
                    graph.add_edge(
                        ...
                        args=argActionBlock(node) if actions else None,
                    )
                    ...
                    if node.DOT():
                        ...
                        graph.add_edge(
                            ...
                            to=graph.add_node(
                                CharsetNode(
                                    rule_id=rule.name, idx=chr_idx, charset=charset.id
                                )
                            ),
                        )
                        chr_idx += 1
        ...
        def build_prequel(node):
            ...
            for prequelConstruct in (
                ...
                for identifier in (
                    ...
                    assert (
                        identifier.TOKEN_REF() is not None
    ...
    def _analyze_graph(graph, root=None):
        ...
        logger.info(
            "\tThe farthest rule from %r is %r (%d steps).",
            root,
            farthest_ident,
            max_distance,
        )
        if unreachable_rules:
            logger.warning(
                ...
                root,
                ", ".join(map(repr, unreachable_rules)),
            )
                        ...
                        inf_alts.append(
                            ", ".join(
                                map(
                                    str,
                                    [
                                        graph.vertices[alternative_node.rule_id],
                                        node,
                                        alternative_node,
                                    ],
                                )
        ...
        if inf_alts:
            logger.warning(
                "\t%d alternative(s) with infinite derivation (rule node, alternation node, alternative node):\n\t%s",
                len(inf_alts),
                ",\n\t".join(inf_alts),
            )
        if inf_rules:
