...
class SynthFuzzGeneratorTool:
    ...
    def __init__(
        ...
        edit_seed=None,
        edit_log=None,
        max_inserts_per_quantifier=20,
        save_to_file=True,
        driver=None,
        save_errors_only=False,
        ...
        if self._save_to_file:
            ...
            if self._save_errors_only:
                ...
                if (
                    retcode == 0
                    and index % 1000 == 0
                    and self._population
                    and self._keep_trees
                ):
    ...
    def generate(self, *, rule=None, max_depth=None):
        ...
        if not hasattr(start_rule, "min_depth"):
            logger.warning(
                "The 'min_depth' property of %s is not set. Fallback to 0.", rule
            )
        elif start_rule.min_depth > max_depth:
            raise ValueError(
                f"{rule} cannot be generated within the given depth: {max_depth} (min needed: {start_rule.min_depth})."
            )
    ...
    def mutate(self, mutated_node):
        ...
        while node.parent:
            node = node.parent
        return MutateResult(
            mutant=node, mutated_node=mutated_node, original_node=original_node
        )
    ...
    def insert(self, recipient_tree: DefaultTree, donor_tree: DefaultTree):
                    ...
                    if isinstance(match_node, str):
                        ...
                        if match_node.max != math.inf:
                            possible_locs = possible_locs[
                                : match_node.max - num_matches
                            ]
                        insertion_locations[match_node] = possible_locs
                    else:
                        raise ValueError(
                            f"Unexpected match node type: {type(match_node)}"
                        )
            ...
            for recipient_parent in recipient_parents:
                ...
                for spec, locs in insertion_locations.items():
                    ...
                    for loc in locs:
                        ...
                        donor_node = random.choice(
                            list(donor_tree.nodes_by_name[spec.rule_name])
                        )
                        # donor_node = list(donor_tree.nodes_by_name[spec.rule_name])[0]
                        # Make sure that the ancestors and siblings match
                        if not (
                            self._population.context_filter.verify_k_ancestors(
                                recipient_node, donor_node
                            )
                            and self._population.context_filter.verify_l_siblings(
                                recipient_node, donor_node
                            )
                            and self._population.context_filter.verify_r_siblings(
                                recipient_node, donor_node
                            )
                        ):
                            continue
        ...
        return InsertResult(
            ...
            substitutions=None,
            is_fit=False,
            fitness_violation=FitnessViolation.NO_INSERT_LOC,
        )
    ...
    def index_nodes(self, current, nodes_by_name, exclude_subtree):
        ...
        if current.name in self._parameter_blacklist and (
            self._parameter_blacklist[current.name] == "*"
            or current.parent.name in self._parameter_blacklist[current.name]
        ):
            return
        if (
            "*" in self._parameter_blacklist
            and current.name in self._parameter_blacklist["*"]
        ):
            return
    ...
    def edit(self, recipient_node, donor_node):
        ...
        def get_matching_nodes(ref_value, value_list, node_list):
            return [
                node_list[i] for i, value in enumerate(value_list) if value == ref_value
            ]
        ...
        for name in common_names:
            ...
            for unique_node_string in set(node_strings_in_fragment):
                param_nodes_in_fragment = get_matching_nodes(
                    ref_value=unique_node_string,
                    value_list=node_strings_in_fragment,
                    node_list=nodes_in_fragment,
                )
                param_nodes_in_context = get_matching_nodes(
                    ref_value=unique_node_string,
                    value_list=node_strings_in_context,
                    node_list=nodes_in_context,
                )
                    ...
                    if a_node.name == c_node.name:
                        if a_node in parameters:
                            ...
                            matching_nodes.append(
                                NodePair(concrete=c_node, abstract=a_node)
                            )
                        break
        ...
        def recursively_match_nodes(
            abstract_nodes: list[Rule], concrete_nodes: list[Rule]
        ):
            matching_nodes = match_nodes(
                abstract_nodes=abstract_nodes, concrete_nodes=concrete_nodes
            )
            for pair in matching_nodes:
                if pair.abstract.children is None or pair.concrete.children is None:
                    continue
                recursively_match_nodes(
                    abstract_nodes=pair.abstract.children,
                    concrete_nodes=pair.concrete.children,
                )

        # get parameters
        assert len(ancestors_concrete) == len(ancestors_abstract)
        for ancestor_idx in range(1, len(ancestors_concrete)):
            siblings_concrete_left, siblings_concrete_right = get_siblings(
                ancestor_idx, ancestors_concrete
            )
            siblings_abstract_left, siblings_abstract_right = get_siblings(
                ancestor_idx, ancestors_abstract
            )
            recursively_match_nodes(
                abstract_nodes=siblings_abstract_left,
                concrete_nodes=siblings_concrete_left,
            )
            recursively_match_nodes(
                abstract_nodes=siblings_abstract_right,
                concrete_nodes=siblings_concrete_right,
            )
        ...
        for param_nodes in parameters.values():
            for param_node in param_nodes:
                if param_node.name in self._fitness_should_sub and (
                    self._fitness_should_sub[param_node.name] == "*"
                    or param_node.parent.name
                    in self._fitness_should_sub[param_node.name]
                ):
        ...
        def has_duplicates(node):
            if node.name in self._fitness_no_dupes and (
                self._fitness_no_dupes[node.name] == "*"
                or node.parent.name in self._fitness_no_dupes[node.name]
            ):
