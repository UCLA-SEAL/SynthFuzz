...
from grammarinator.cli import (
    add_encoding_argument,
    add_encoding_errors_argument,
    add_jobs_argument,
    import_list,
    init_logging,
    logger,
...
from inators.arg import (
    add_log_level_argument,
    add_sys_path_argument,
    add_sys_recursion_limit_argument,
    add_version_argument,
    process_log_level_argument,
    process_sys_path_argument,
    process_sys_recursion_limit_argument,
)
    ...
    return SynthFuzzGeneratorTool(
        ...
        population=(
            SynthFuzzPopulation(
                args.population,
                min_depths={
                    name: method.min_depth
                    for name, method in inspect.getmembers(
                        args.generator, inspect.ismethod
                    )
                    if hasattr(method, "min_depth")
                },
        ...
        max_inserts_per_quantifier=args.max_inserts,
...
def execute():
    parser = ArgumentParser(
        description="Grammarinator: Generate",
        epilog="""
    ...
    parser.add_argument(
        "--population", metavar="DIR", help="directory of grammarinator tree pool."
    )
    parser.add_argument(
        "--no-generate",
        dest="generate",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="FILE",
        help="Pickle file containing insert patterns.",
    )
    parser.add_argument(
        "--mutation-config",
        metavar="FILE",
        default=None,
        type=Path,
        help="TOML file containing mutation config.",
    )
    ...
    parser.add_argument(
        "--batch-dir", metavar="DIR", help="directory to store batched tests."
    )
    parser.add_argument(
        "--batch-ext",
        default="",
        help='File extension for batched tests including the "." (example: ".txt").',
    )
    parser.add_argument(
        "--test-output-path",
        default=None,
        help="path to the directory where the test output will be saved.",
    )
    parser.add_argument(
        "--save-errors-only",
        action="store_true",
        help="save only tests that cause the test driver to return non-zero.",
    )
    parser.add_argument(
        "--driver-class",
        default=None,
        type=str,
        help="fully qualified name of the test driver class.",
    )
    parser.add_argument(
        "--driver-config",
        default=None,
        metavar="FILE",
        help="TOML file containing driver config.",
    )
    parser.add_argument(
        "--fitness-log-only",
        action="store_true",
        help="Only log fitness values instead of applying.",
    )
    parser.add_argument(
        "--disable-parameters",
        action="store_true",
        help="Disable parameters during generation.",
    )
    ...
    parser.add_argument(
        ...
        "--out",
        metavar="FILE",
        default=join(os.getcwd(), "tests", "test_%d"),
    ...
    if args.batch_size > 1:
        ...
        if args.batch_dir is None:
            raise ValueError(
                f"Batch directory must be specified if batch size is > 1. (batch_size={args.batch_size})"
            )
        elif exists(args.batch_dir) and not isdir(args.batch_dir):
            raise ValueError(
                f"Batch directory must be a directory if batch size is > 1. (batch_dir={args.batch_dir})"
            )
    ...
    if args.jobs > 1:
        with Manager() as manager:
            with generator_tool_helper(
                args,
                weights=manager.dict(args.weights),
                lock=manager.Lock(),
                save_to_file=save_to_file,
            ) as generator_tool:  # pylint: disable=no-member
                parallel_create_test = partial(
                    create_test, generator_tool, seed=args.random_seed
                )
                with Pool(args.jobs) as pool:
                    if args.batch_size > 1:
                        ...
                        for idx, _ in enumerate(
                            pool.imap_unordered(
                                parallel_create_test,
                                count(0) if args.n == inf else range(args.n),
                            )
                        ):
...
def batched_run(pool, parallel_create_test, args):
    last_idx = 0
    test_batch = []
    for test, index in pool.imap(
        parallel_create_test, count(0) if args.n == inf else range(args.n)
    ):
