...
// Dimensions
UNKNOWN_DIMENSION_ENTRY : '?x' ;
KNOWN_DIMENSION_ENTRY : DIGITS 'x' ;
dimension_entry : UNKNOWN_DIMENSION_ENTRY | KNOWN_DIMENSION_ENTRY ;
static_dimension_list   : KNOWN_DIMENSION_ENTRY+ ;
...
attribute_alias : '#' (string_literal | BARE_ID | 'loc') ;
...
// Vector types
vector_element_type : float_type | integer_type | 'index';
...
pretty_dialect_item : BARE_ID pretty_dialect_item_body? 
                    | BARE_ID '.' (BARE_ID | NONE_TYPE_LITERAL) pretty_dialect_item_body? ;
...
pretty_dialect_item_contents : ('(' pretty_dialect_item_contents ')')
                              ...
                              | stride_list
                              | type ;
// add in CIRCT's non-standard dialect type
hw_modty_content_item : BARE_ID | 'x' | ':' | type | ',';
dialect_type : '!' (opaque_dialect_item | pretty_dialect_item)
             | 'uint<' DIGITS '>' 
             | '!firrtl.uint<' DIGITS '>'
             | '!hw.modty<' hw_modty_content_item* '>' ;
...
type              : type_no_loc | type_with_loc ;
type_no_loc : type_alias | dialect_type | standard_type ;
type_with_loc : type_no_loc trailing_location ;
...
// Location attribute
location_attribute : trailing_location | BARE_ID '=' trailing_location;

// Generic attribute
generic_attribute_value : BARE_ID generic_attribute_content? ;
generic_attribute_content : '<' generic_attribute_content_entry* '>' ;
generic_attribute_content_entry : type | ':' | ',' | dimension_list | DIGITS ;

// Attribute values
attribute_value : attribute_alias | dialect_attribute | standard_attribute | location_attribute | generic_attribute_value;
unit_attribute_entry : 'value' ;
dependent_attribute_entry : (BARE_ID | 'module' | unit_attribute_entry) '=' attribute_value ;
...
trailing_location : ('loc' '(' (location | attribute_alias) ')') ;
