...
def convert_single_onnx_to_mlir(input_file: Path, output_folder: Path):
    ...
    if predicted_output.exists():
        return
    subprocess.run(['/workdir/onnx-mlir/build/Debug/bin/onnx-mlir', '--EmitMLIR', str(input_file), '-o', str(output_path_prefix)], check=False)
...
if __name__ == "__main__":
    ...
    output_folder = Path("/workdir/mlir-eval/neuri/mlir")
